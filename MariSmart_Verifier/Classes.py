...
class MaritimeSystem(object):
    def __init__(self, _shipper=None, _carrier=None, _consignee=None, _inspector=None, _exporter=None, _importer=None) -> None:
        ...
        self.clock_set = set(['create_timeCLK', 'depart_timeCLK',
                             'arrive_timeCLK', 'receive_timeCLK'])  # 存放时钟变量，形如：myCLK
        ...
        self.contract_dict = {'shipment': None, 'shipper': _shipper, 'carrier': _carrier,
                              'consignee': _consignee, 'pre_shipment_inspector': _inspector, 'export_port_operator': _exporter, 'import_port_operator': _importer}
...
class MaritimeFunction(object):
    def __init__(self, _name_str='', _require_list=[], _stmt_str='', _para_list=[], _return_str='void', _timed_condition_list=[], timed_reset_list=[]) -> None:
        ...
        self.shipment_function_list = []  # 可能调用的shipment函数列表
        self.is_activate = False  # 是否被激活
...
carrier = MaritimeContract('carrier', [
                           sign, withdraw, cancel, close_carrier, depart, reportLoss, reportDamage, arrive, rearrange, compensate])

receiveShipment = MaritimeFunction('receiveShipment', ['status==status_imported && arrive_timeCLK<=receive_valid'],
                                   'void receiveShipment(bool _is_damaged){\nshipment_receiveShipment(_is_damaged);\n}', [], 'void', [], ['receive_timeCLK'])
