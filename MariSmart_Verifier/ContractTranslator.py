        ...
        if expression_json is not None:
            ...
            if var_type == "BooleanLiteral":
                ...
                if ".transfer" in function_name or ".send" in function_name or ".call.value" in function_name:
                    ...
                    return ''
                elif function_name.strip(' ').startswith('shipment_get'):
                ...
                elif function_name.strip(' ').startswith('shipment_set'):
                    # deal with setter function
                    setter_str = function_name.strip(' ')[12:]
                    setter_str = re.sub(r'([A-Z])', r'_\1', setter_str).lower()
                    if para_str == "":
                        getter_str = getter_str.replace(
                            "create_time", "create_timeCLK")
                        getter_str = getter_str.replace(
                        ...
                        getter_str = getter_str.replace(
                            "receive_time", "receive_timeCLK")
                        return '%s = 0' % setter_str[1:]
                    else:
                        return '%s = %s' % (setter_str[1:], para_str)
                elif function_name.strip(' ').startswith('shipment_') and not function_name.strip(' ').startswith('shipment_externalTransfer'):
                    # deal with shipment transition function
                    self.function.shipment_function_list.append(
                        function_name.strip(' ')[9:])
                    return 'call_%s = true; ' % function_name.strip(' ')[9:]+function_name + "(" + para_str + ")"
                else:
    ...
    def translate(self) -> MaritimeSystem:
        ...
        for contract_reader in self.reader.contract_list:
            ...
            if contract_reader.name_str == 'shipper':
                ...
                contract = self.system.contract_dict['pre_shipment_inspector']
            elif contract_reader.name_str == 'export_port_operator':
                contract = self.system.contract_dict['export_port_operator']
            elif contract_reader.name_str == 'import_port_operator':
                contract = self.system.contract_dict['import_port_operator']
            else:
                continue
            ...
            for function_json in contract_reader.function_list:
                ...
                # debug: 此处应重置 self.function, 否则未知名字函数将覆盖上一个函数
                self.function = None
                for function in contract.function_list:
                    if function.name_str == name_str:
                        self.function = function
                        self.function.is_activate = True
                        break
                    ...
                    for para in function_json['parameters']['parameters']:
                        if 'name' not in para['typeName'].keys():
                            ...
                            self.function.para_list.append(
                                ['int', para['name'], '0', str(MAX_PARA_VALUE)])
