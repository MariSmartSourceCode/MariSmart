...
library Shipment {
    event ShipmentCarrierSet(
        address indexed msgSender,
        bytes16 indexed shipmentUuid,
        address carrier
    );
    event ShipmentModeratorSet(
        address indexed msgSender,
        bytes16 indexed shipmentUuid,
        address moderator
    );
    ...
    function setCarrier(
        Data storage self,
        bytes16 _shipmentUuid,
        address _carrier
    ) internal isShipper(self, "Only Shipper allowed to set Carrier") {
        require(
            self.state == State.CREATED,
            "Carrier can only be modified in Created state"
        );
    ...
    function setModerator(
        ...
        bytes16 _shipmentUuid,
        address _moderator
    ) internal isShipper(self, "Only Shipper allowed to set Moderator") {
        require(
            self.state == State.CREATED,
            "Moderator can only be modified in Created state"
        );
        self.moderator = _moderator;
        emit ShipmentModeratorSet(msg.sender, _shipmentUuid, _moderator);
    }

    function setInProgress(Data storage self, bytes16 _shipmentUuid) internal {
        require(
            self.carrier != address(0),
            "Carrier must exist before marking a shipment In Progress"
        );
        require(
            msg.sender == self.carrier || msg.sender == self.moderator,
            "Only Carrier or Moderator allowed to set In Progress"
        );
        require(
            self.state == State.CREATED,
            "Only Created shipments can be marked In Progress"
        );
        self.state = State.IN_PROGRESS;
        emit ShipmentInProgress(msg.sender, _shipmentUuid);
    }

    function setComplete(Data storage self, bytes16 _shipmentUuid) internal {
        require(
            msg.sender == self.carrier || msg.sender == self.moderator,
    ...
    function setCanceled(Data storage self, bytes16 _shipmentUuid) internal {
        require(self.state != State.CANCELED, "Already canceled");
        require(
            self.state != State.CREATED ||
                msg.sender == self.shipper ||
                msg.sender == self.carrier ||
                msg.sender == self.moderator,
        ...
        require(
            self.state != State.COMPLETE || msg.sender == self.moderator,
