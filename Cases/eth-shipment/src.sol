...
contract ShipmentTracking {
    ...
    event Departed(string location, string custodian, uint time, uint index);

    event Arrived(string location, string custodian, uint time, uint index);
     ...
     */
    function getLocations() public constant returns (Location[]) {
        return locations;
    }
}
