...
class SolidityFileReader(object):
    def __init__(self, _source_file_path):
        ...
        if not find_flag:
            raise Exception('No solidity file found in %s' %
    ...
    def pretreat(self, solStr) -> str:
        solStr = solStr.replace(
            'FormattedShipment(shipments[_UID]).', 'shipment_')
        solStr = solStr.replace('FormattedShipment(shipment).', 'shipment_')
        solStr = solStr.replace(
            'FormattedShipment(shipments[_UID]).', 'shipment_')
        solStr = solStr.replace('address _shipment,', '')
        solStr = solStr.replace('address _shipment', '')
        solStr = solStr.replace(
            '{value: escrow_amount}', '')
        solStr = solStr.replace(
        ...
        solStr = solStr.replace('uint _escrow_amount,', '')
        solStr = solStr.replace('uint _escrow_amount', '')
        solStr = solStr.replace('uint _UID,', '')
        solStr = solStr.replace('uint _UID', '')
        solStr = solStr.replace('FormattedShipment(_shipment).', 'shipment_')
        ...
        solStr = solStr.replace("address", "int")
        ...
        solStr = solStr.replace("memory", "")
        solStr = solStr.replace("string", "bool")
        ...
        solStr = solStr.replace('keccak256', '')
        solStr = solStr.replace('abi.encodePacked', '')
...
if __name__ == '__main__':
    ...
    # reader = SolidityFileReader('./Cases/IoT')
